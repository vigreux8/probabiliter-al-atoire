...
class TypeProbabiliter(CalculculeMathematique):
    ...
    def combinaison(self):
        ...
        dividende = self.factoriels(self.cardinale_n)
        diviseur = self.factoriels(
            self.nb_tirage_p)*self.factoriels(self.cardinale_n-self.nb_tirage_p)
        self.set_resultat(dividende/diviseur)

    def p_suite(self):
        self.model_selectionnner = "P_suite"
        dividende = self.factoriels(self.cardinale_n+self.nb_tirage_p-1)
        diviseur = self.factoriels(self.nb_tirage_p) * \
            self.factoriels(self.cardinale_n-1)
        self.set_resultat(dividende/diviseur)
    ...
    def set_random_parametre(self):
        ...
        if self.random_1_ou_2():
            ...
            if not self.bool_remise:
                self.cardinale_n = random.randint(
                    1, ConstParamatreApplication.HASART_MAXIMUM)
                self.nb_tirage_p = random.randint(1, self.cardinale_n+1)
            else:
                self.cardinale_n = random.randint(
                    1, ConstParamatreApplication.HASART_MAXIMUM)
                self.nb_tirage_p = random.randint(
                    1, ConstParamatreApplication.HASART_MAXIMUM)
            ...
            if self.distingable == "t":
                ...
                for i in range(0, random.randint(1, 6)):
                    self.list_n_permutation_partiel.append(
                        random.randint(1, 5))
...
class UsingTerminal(CalculeProbabiliter):
    """
    ...
    def terminal_explain_why(self):
        if ConstParamatreApplication.EXPLAIN_WHY:
            if self.distingable:
                print(
                    f"_____pourquoi se choix ?______ \n les element sont distingable {self.distingable_mot_complet} les formule comprenne que la cardinaliter")
            else:
                remise = self.converte_True_False_to_str(
                    self.bool_remise, "oui", "non")
                ordre = self.converte_True_False_to_str(
                    self.bool_ordre, "oui", "non")

                print(
                    f"_____pourquoi se choix ?_____ \n-les element ne sont pas ditingable la remise et importante \n-remise = {remise} \n-l'ordre = {ordre}")
...
class UsingStreamlit(CalculeProbabiliter):
    def __init__(self) -> None:
        super().__init__()
    ...
    def display_streamlit_resultat(self):
        with self.colonne_centralle:
            ...
            texte_html = re.sub(f'({self.model_selectionnner})',
                                r'<span style="background-color: blue;">\1</span>', texte)
            st.write(texte_html, unsafe_allow_html=True)
            st.write(f"-il y : {self.resultat} possibilité.")
            if self.distingable == "p":
                st.write(
                    f"-la taille du groupe et de : {sum(self.list_n_permutation_partiel)}")

    def display_streamlit_avertissement(self):
        with self.colonne_centralle:
            if not self.bool_remise and not self.bool_distingable:
                st.write(
                    "<p style='color: red; font-weight: bold;'>nombre d'élement ne peut être supérieur au tirage !</p>", unsafe_allow_html=True)

    def display_streamlit_information(self):
        with self.colonne_gauche:
            if self.distingable == False:
                st.write(
                    f"remise : {self.converte_True_False_to_str(self.bool_remise,'oui','non')}")
                st.write(
                    f"ordre : {self.converte_True_False_to_str(self.bool_ordre,'oui','non')}")
            ...
            if self.distingable != False:
                ...
                st.write(
                    f"distingable : {self.converte_True_False_to_str(self.distingable, si_vrais='t',si_faut='p',message_si_vrais= 'totalement',message_si_faut='partiellement')}")
                if self.distingable == "t":
            ...
            if self.distingable == "p":
                st.write(
                    f"nombre de groupe distingable: {len(self.list_n_permutation_partiel)}")
                st.write(
                    f"nombre de personne par groupe : {self.list_n_permutation_partiel}")
                pass

    def what_streamlit_app_choice(self):
        with self.colonne_gauche:
            selecte_option_choice = st.selectbox(
                "Options a choisir", ConstParamatreApplication.OPTION_CHOISI)
    ...
    def app_streamlit__distingable(self):
        with self.colonne_gauche:
            Select_box_distingable_type = st.selectbox(
                "les objet sont distingable :", ("PARTIELEMENT", "TOTALEMENT"))
            if Select_box_distingable_type == "PARTIELEMENT":
                self.distingable = "p"
                nb_n_distingable = st.number_input(
                    "Nombre de groupe :", step=1, min_value=2)
                for i in range(0, nb_n_distingable):
                    self.list_n_permutation_partiel.append(st.number_input(
                        f"nombre d'objet du groupe n{i+1}: ", step=1))
                self.cardinale_n = sum(self.list_n_permutation_partiel)
            elif Select_box_distingable_type == "TOTALEMENT":
                self.distingable = "t"
                self.cardinale_n = st.number_input(
                    "Nombre d'objet au total :", step=1, min_value=1)
    ...
    def app_streamlit_ordre_and_tirage(self):
        with self.colonne_gauche:
            ...
            self.cardinale_n = st.number_input(
                "Nombre d'élement dans la liste (cardinale_n) :", step=1, min_value=1)
            if not self.bool_remise:
                self.nb_tirage_p = st.number_input(
                    "Nombre de tirage :", step=1, min_value=1, max_value=self.cardinale_n)
            else:
                self.nb_tirage_p = st.number_input(
                    "Nombre de tirage :", step=1, min_value=1)
